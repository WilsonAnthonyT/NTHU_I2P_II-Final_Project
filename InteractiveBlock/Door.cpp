...
Door::Door(std::string img, float x, float y, DoorState ds) : currState(ds), initState(ds), Engine::Sprite(img, x, y){
...
bool Door::IsCollision(float x, float y) {
    ...
    for (auto& it : scene->InteractiveBlockGroup->GetObjects()) {
        Door* friends = dynamic_cast<Door*>(it);
        // Skip if enemy is not visible, is the healer itself, or invalid
